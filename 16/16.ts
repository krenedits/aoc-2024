...
const traverse = (
    ...
    const distanceKey = `${point.join(',')}` as `${number},${number}`;

    if (
        (distances[distanceKey] && distances[distanceKey] < score) ||
        localMin <= score
    ) {
        const next = [point[0] + direction[0], point[1] + direction[1]];
        const nextDistance = distances[`${next[0]},${next[1]}`];
        if (nextDistance && nextDistance < score + 1) {
